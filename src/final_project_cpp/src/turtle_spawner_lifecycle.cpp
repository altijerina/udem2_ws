...
#include "robot_interfaces/action/move_turtle.hpp"
#include "geometry_msgs/msg/twist.hpp"

using Twist = geometry_msgs::msg::Twist;
using MoveTurtle = robot_interfaces::action::MoveTurtle;
...
private:
    ...
    {       
        (void)uuid;
        RCLCPP_INFO(this->get_logger(), "Recieved a new goal");

        //Check if node is activated
        if (!server_activated_)
        {
            RCLCPP_INFO(this->get_logger(), "Acton server not activated yet");
    ...
    void spawn_turtle()
    {
        ...
        request->y = random_y;
        request->theta = 0.0;
        RCLCPP_INFO(this->get_logger(), "Attempting to spawn turtle: %s", turtle_name_.c_str());
